...
class FAMO(WeightMethod):
    ...
    def __init__(
        ...
        self.w = torch.tensor([0.0] * n_tasks,
                              device=device,
                              requires_grad=True)
...
class NashMTL(WeightMethod):
    ...
    def _stop_criteria(self, gtg, alpha_t):
        return ((self.alpha_param.value is None) or
    ...
    def _init_optim_problem(self):
        ...
        self.prvs_alpha_param = cp.Parameter(shape=(self.n_tasks, ),
                                             value=self.prvs_alpha)
        self.G_param = cp.Parameter(shape=(self.n_tasks, self.n_tasks),
                                    value=self.init_gtg)
        self.normalization_factor_param = cp.Parameter(shape=(1, ),
                                                       value=np.array([1.0]))
        ...
        for i in range(self.n_tasks):
            constraint.append(-cp.log(self.alpha_param[i] *
                                      self.normalization_factor_param) -
                              cp.log(G_alpha[i]) <= 0)
        obj = cp.Minimize(
...
class LinearScalarizationWithSAMO(WeightMethod):
    ...
    def grad2vec(shared_grads, grads, grad_dims, task):
        ...
        for name, grad in shared_grads[task].items():
    ...
    def backward(
        self,
        losses: torch.Tensor,
        shared_grads: Union[str, Union[str, torch.Tensor]],
        ...
        **kwargs,
        ...
        self.get_weighted_loss(shared_grads, shared_parameters)
        if self.max_norm > 0:
            torch.nn.utils.clip_grad_norm_(shared_parameters, self.max_norm)
        return None, {}  # NOTE: to align with all other weight methods


class LinearScalarizationWithSAMOffice(WeightMethod):

    def __init__(self, n_tasks, device: torch.device, max_norm=1.0):
        super().__init__(n_tasks, device=device)
        self.max_norm = max_norm
        self.task_name = ['Art', 'Clipart', 'Product', 'Real_World']
    ...
    def get_weighted_loss(
        ...
        **kwargs,
        ...
        for i in range(self.n_tasks):
            self.grad2vec(shared_grads, grads, grad_dims, i, self.task_name[i])
            # multi_task_model.zero_grad_shared_modules()
            for p in shared_parameters:
                p.grad = None
    ...
    @staticmethod
    def grad2vec(shared_grads, grads, grad_dims, task, name):
        # store the gradients
        grads[:, task].fill_(0.0)
        cnt = 0
        # for mm in m.shared_modules():
        #     for p in mm.parameters():
        for name, grad in shared_grads[name].items():
            grad_cur = grad.data.detach().clone()
            beg = 0 if cnt == 0 else sum(grad_dims[:cnt])
            en = sum(grad_dims[:cnt + 1])
            grads[beg:en, task].copy_(grad_cur.data.view(-1))
...
class MGDA(WeightMethod):
    ...
    def _flattening(grad):
        return torch.cat(
            tuple(g.reshape(-1, ) for i, g in enumerate(grad)),
            dim=0,
        )
    ...
    def get_weighted_loss(
        ...
        **kwargs,
        ...
        return weighted_loss, dict(
            weights=torch.from_numpy(sol.astype(np.float32)))


class MGDAWithSAMO(WeightMethod):
    ...
    def get_weighted_loss(
        ...
        **kwargs,
        ...
        for i in range(self.n_tasks):
            grads[i] = [
                torch.flatten(grad) for grad in shared_grads[i].values()
            ]
        ...
        sol = torch.tensor(sol, dtype=grads.dtype, device=grads.device)
        g = (grads * sol).sum(dim=1)
        self.overwrite_grad(shared_parameters, g, grad_dims)
        return None
...
class MGDAWithSAMOffice(WeightMethod):
    ...
    def get_weighted_loss(
        ...
        **kwargs,
        ...
        for i in range(self.n_tasks):
            grads[i] = [
                torch.flatten(grad)
                for grad in shared_grads[self.task_name[i]].values()
            ]
...
class LOG_MGDA(WeightMethod):
    ...
    def get_weighted_loss(
        ...
        **kwargs,
        ...
        #sol = sol * self.n_tasks  # make sure it sums to self.n_tasks
        c = sum(
            [sol[i] / (losses[i] + 1e-8).detach() for i in range(len(sol))])
        weighted_loss = sum([(losses[i] + 1e-8).log() * sol[i] / c
...
class Uncertainty(WeightMethod):
    ...
    def __init__(self, n_tasks, device: torch.device):
        super().__init__(n_tasks, device=device)
        self.logsigma = torch.tensor([0.0] * n_tasks,
                                     device=device,
                                     requires_grad=True)
    ...
    def get_weighted_loss(self, losses: torch.Tensor, **kwargs):
        ...
        return loss, dict(weights=torch.exp(
            -self.logsigma))  # NOTE: not exactly task weights
    ...
    def _project_conflicting(self, grads: List[Tuple[torch.Tensor]]):
        ...
        for g_i in pc_grad:
            ...
            for g_j in grads:
                ...
                if g_i_g_j < 0:
                    g_j_norm_square = (torch.norm(
                        torch.cat([torch.flatten(g) for g in g_j]))**2)
                    for grad_i, grad_j in zip(g_i, g_j):
...
class PCGradWithSAMO(WeightMethod):
    """Modification of: https://github.com/WeiChengTseng/Pytorch-PCGrad/blob/master/pcgrad.py
    ...
    def _set_pc_grads(self,
        ...
        #         torch.autograd.grad(l, shared_parameters, retain_graph=True)
        #     )
        for task in shared_grads.keys():
            _shared_grad = list(shared_grads[task].values())
            _shared_grads.append(_shared_grad)

        if isinstance(shared_parameters, torch.Tensor):
            shared_parameters = [shared_parameters]
        non_conflict_shared_grads = self._project_conflicting(_shared_grads)
        for p, g in zip(shared_parameters, non_conflict_shared_grads):
            p.grad = g
    ...
    def backward(
        ...
        **kwargs,
        ...
        self._set_pc_grads(shared_grads, shared_parameters,
                           task_specific_parameters)
        # make sure the solution for shared params has norm <= self.eps
        if self.max_norm > 0:
            torch.nn.utils.clip_grad_norm_(shared_parameters, self.max_norm)
...
class CAGrad(WeightMethod):
    ...
    def get_weighted_loss(
        ...
        **kwargs,
        ...
        for i in range(self.n_tasks):
            ...
            for p in shared_parameters:
                grad_data[i].append(p.grad.data.clone().detach().cpu().numpy())
    ...
    def cagrad(self, grads, alpha=0.5, rescale=1):
        ...
        def objfn(x):
            return (x.reshape(1, self.n_tasks).dot(A).dot(
                b.reshape(self.n_tasks, 1)) + c * np.sqrt(
                    x.reshape(1, self.n_tasks).dot(A).dot(
                        x.reshape(self.n_tasks, 1)) + 1e-8)).sum()
    ...
    def backward(
        ...
        **kwargs,
        ...
        GTG, w, grad_data = self.get_weighted_loss(losses, shared_parameters)
        if self.max_norm > 0:
            torch.nn.utils.clip_grad_norm_(shared_parameters, self.max_norm)
        return None, {
            "GTG": GTG,
...
class CAGradWithSAMO(WeightMethod):
    ...
    def backward(
        ...
        **kwargs,
        ...
        GTG, w = self.get_weighted_loss(shared_grads, shared_parameters)
        if self.max_norm > 0:
            torch.nn.utils.clip_grad_norm_(shared_parameters, self.max_norm)
        return None, {
            "GTG": GTG,
...
class FairGrad(WeightMethod):
    ...
    def get_weighted_loss(
        ...
        **kwargs,
        ...
        # rep = kwargs.get('representation')
        # g_rep = [torch.autograd.grad(losses[i], rep, retain_graph=True)[0] for i in range(self.n_tasks)]
        # g_rep = torch.stack(g_rep, dim=0).permute(1, 2, 0)
        # ipdb.set_trace()
...
class FairGradWithSAMO(WeightMethod):
...
class FairGradWithSAMOffice(WeightMethod):
...
class GradDrop(WeightMethod):
    ...
    def get_weighted_loss(
        ...
        **kwargs,
        ...
        M = P.gt(U).view(-1, 1) * grads.gt(0) + P.lt(U).view(-1,
                                                             1) * grads.lt(0)
...
class IMTLG(WeightMethod):
    ...
    def get_weighted_loss(
        ...
        **kwargs,
        ...
        alpha = torch.cat(
            (torch.tensor(1 - alpha_.sum(),
                          device=self.device).unsqueeze(-1), alpha_))
...
class DynamicWeightAverage(WeightMethod):
    ...
    def get_weighted_loss(self, losses, **kwargs):
        ...
        if self.running_iterations > self.iteration_window:
            ws = self.costs[self.iteration_window:, :].mean(
                0) / self.costs[:self.iteration_window, :].mean(0)
            self.weights = (self.n_tasks * np.exp(ws / self.temp)) / (np.exp(
                ws / self.temp)).sum()
...
class WeightMethods:
    ...
    def __init__(self, method: str, n_tasks: int, device: torch.device,
                 **kwargs):
        """
        :param method:
        """
    ...
    def backward(
            self, losses,
            **kwargs) -> Tuple[Union[torch.Tensor, None], Union[Dict, None]]:
...
METHODS = dict(
    ...
    lswithsamoffice=LinearScalarizationWithSAMOffice,
    lswithsamo=LinearScalarizationWithSAMO,
    uw=Uncertainty,
    scaleinvls=ScaleInvariantLinearScalarization,
    rlw=RLW,
    dwa=DynamicWeightAverage,
    pcgrad=PCGrad,
    pcgradwithsamo=PCGradWithSAMO,
    mgda=MGDA,
    mgdawithsamo=MGDAWithSAMO,
    mgdawithsamoffice=MGDAWithSAMOffice,
    graddrop=GradDrop,
    log_mgda=LOG_MGDA,
    cagrad=CAGrad,
    cagradwithsamo=CAGradWithSAMO,
    ...
    famo=FAMO,
    fairgrad=FairGrad,
    fairgradwithsamo=FairGradWithSAMO,
    fairgradwithsamoffice=FairGradWithSAMOffice,
)
