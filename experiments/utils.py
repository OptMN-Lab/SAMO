...
common_parser.add_argument("--batch-size",
                           type=int,
                           default=120,
                           help="batch size")
common_parser.add_argument("--method",
                           type=str,
...
common_parser.add_argument(
    ...
    default=0.025,
    help=
    "lr for weight method params. If None, set to args.lr. For uncertainty weighting",
)
...
# NashMTL
common_parser.add_argument("--nashmtl-optim-niter",
                           type=int,
                           default=20,
                           help="number of CCCP iterations")
common_parser.add_argument(
...
common_parser.add_argument("--c",
                           type=float,
                           default=0.4,
                           help="c for CAGrad alg.")
# fairgrad
common_parser.add_argument("--alpha",
                           type=float,
                           default=2.0,
                           help="alpha for FairGrad alg.")
# famo
common_parser.add_argument("--gamma",
                           type=float,
                           default=0.01,
                           help="gamma of famo")
common_parser.add_argument("--use_log",
                           action='store_true',
                           help="whether use log for famo")
common_parser.add_argument("--max_norm",
                           type=float,
                           default=1.0,
                           help="beta for RMS_weight alg.")
common_parser.add_argument("--task",
                           type=int,
                           default=0,
                           help="train single task number for (celeba)")
# dwa
common_parser.add_argument(
    "--dwa-temp",
    type=float,
    default=2.0,
    help=
    "Temperature hyper-parameter for DWA. Default to 2 like in the original paper.",
)
...
def extract_weight_method_parameters_from_args(args):
    ...
    weight_methods_parameters.update(
        dict(
            ...
            cagradwithsamo=dict(c=args.c, max_norm=args.max_norm),
            famo=dict(gamma=args.gamma,
                      w_lr=args.method_params_lr,
                      max_norm=args.max_norm),
            fairgrad=dict(alpha=args.alpha, max_norm=args.max_norm),
            fairgradwithsamo=dict(alpha=args.alpha, max_norm=args.max_norm),
        ))
